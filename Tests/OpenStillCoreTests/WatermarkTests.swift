...
@Suite struct WatermarkTests {
    @Test func vectorDesignRetainsExactTextAndExports()throws {
        ...
        var design=LogoDesign(name:"Arda & Noah",tagline:"Light < moments >");design.color=LogoColor("#FFFFFF")
        let vector=try LogoRenderer.vector(design),svg=vector.svg(title:design.name+" · "+design.tagline)
        #expect(svg.contains("Arda &amp; Noah"));#expect(svg.contains("&lt; moments &gt;"));#expect(!svg.contains("<text"))
    ...
    @Test func realOfflineModel()throws {
        ...
        let start=Date(),results=try LogoInference().generate(helper:URL(fileURLWithPath:helper),model:URL(fileURLWithPath:model),name:"Noah Helms",tagline:"Photography",style:"Minimal, refined automotive photography",symbol:"aperture or monogram",color:LogoColor("#FFFFFF"),accent:LogoColor("#D8C5A6"))
        #expect(results.count==3);#expect(results.allSatisfy{$0.name=="Noah Helms"});print("Local logo model: \(Date().timeIntervalSince(start)) seconds")
