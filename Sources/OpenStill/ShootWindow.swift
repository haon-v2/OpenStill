...
private final class ShootCell:NSCollectionViewItem {
    let preview=NSImageView(),caption=NSTextField(labelWithString:""),rating=NSTextField(labelWithString:"")
    ...
    override func loadView(){
        ...
        for child in [preview,caption,rating]{child.translatesAutoresizingMaskIntoConstraints=false;view.addSubview(child)}
    ...
    func configure(_ item:ShootItem){
        ...
        view.setAccessibilityLabel(caption.stringValue+", \(item.record.rating) stars, "+item.record.flag.rawValue);view.toolTip=caption.stringValue
...
final class ShootWindow:NSWindowController,NSCollectionViewDataSource,NSCollectionViewDelegate,NSWindowDelegate {
    ...
    private let minimum=NSPopUpButton(frame:.zero,pullsDown:false),flag=NSPopUpButton(frame:.zero,pullsDown:false),sort=NSPopUpButton(frame:.zero,pullsDown:false)
    ...
    init(urls:[URL], embedded:Bool = false, selectedURL:URL? = nil){
        ...
        for (control,title) in [(minimum,"Minimum star rating"),(flag,"Flag filter"),(sort,"Sort photographs")]{control.target=self;control.action = #selector(filterChanged);control.setAccessibilityLabel(title)}
        search.placeholderString="Search filenames";search.target=self;search.action = #selector(filterChanged);search.sendsSearchStringImmediately=true;search.setAccessibilityLabel("Search library filenames")
        ...
        if embedded {
            ...
            for (title, action) in [("Edit selected",#selector(openSelected)),("Compare two",#selector(compareSelected)),("Copy adjustments",#selector(copyAdjustments)),("Paste adjustments…",#selector(pasteAdjustments)),("Undo batch",#selector(undoBatch)),("Export selected…",#selector(exportSelection)),("Refresh",#selector(refreshAction))] {
                let item=NSMenuItem(title:title,action:action,keyEquivalent:"");item.target=self;more.menu?.addItem(item)
            }
            top=NSStackView(views:[minimum,flag,sort,NSView(),more])
        } else { top=NSStackView(views:[minimum,flag,sort,NSView(),open,compare,refreshButton]) }
        top.spacing=8
        ...
        let hint=NSTextField(labelWithString:"0–5 rate · P pick · X reject · U unflag · ⌘-click selects multiple");hint.font = .systemFont(ofSize:10);hint.textColor = .secondaryLabelColor
        ...
        grid.mark = {[weak self] rating,flag in self?.mark(rating:rating,flag:flag)};grid.openSelection = {[weak self] in self?.openSelected()}
    ...
    private func applyFilter(preserving selection:Set<UUID>){
        ...
        shown=ShootWorkflow.filter(all.filter { query.isEmpty || $0.url.lastPathComponent.localizedCaseInsensitiveContains(query) },minimumRating:max(0,minimum.indexOfSelectedItem),flag:ShootFlagFilter.allCases[max(0,flag.indexOfSelectedItem)],sort:ShootSort.allCases[max(0,sort.indexOfSelectedItem)])
    ...
    private func mark(rating:Int? = nil,flag:PhotoFlag? = nil){
        let selected=selectedItems;guard !selected.isEmpty else{message.stringValue="Select one or more photographs first.";return}
        do{
            for item in selected{let updated=try ShootWorkflow.mark(item.id,rating:rating,flag:flag);if let i=all.firstIndex(where:{$0.id==item.id}){all[i].record=updated}}
    ...
    func collectionView(_ collectionView:NSCollectionView,itemForRepresentedObjectAt indexPath:IndexPath)->NSCollectionViewItem{
        ...
        let operation=BlockOperation();operation.addExecutionBlock{[weak self,weak cell,weak operation] in
            ...
            let result=try? autoreleasepool{try ModernRenderer.display(ModernRenderer.render(source:item.url,recipe:item.record.active.recipe,maximumDimension:420))}
            guard operation?.isCancelled==false else{return}
