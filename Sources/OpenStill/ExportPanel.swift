...
final class ExportPanel:NSWindowController,NSWindowDelegate,NSTextFieldDelegate {
    ...
    private let hdrOutput=NSPopUpButton(),preset=NSPopUpButton(),format=NSPopUpButton(),profile=NSPopUpButton(),depth=NSPopUpButton(),intent=NSPopUpButton()
    ...
    init(items:[ShootItem]) {
        ...
        format.addItems(withTitles:["JPEG","PNG","TIFF","HEIF"]);hdrOutput.addItems(withTitles:["SDR"]+HDRExport.allCases.map(\.title));profile.addItems(withTitles:ExportProfile.allCases.map(\.title));depth.addItems(withTitles:["8 bit","16 bit"]);intent.addItems(withTitles:RenderingIntent.allCases.map(\.title));intent.selectItem(at:1)
        edge.placeholderString="Original dimensions";edge.delegate=self;name.delegate=self
        let grid=NSGridView(views:[[NSTextField(labelWithString:"Format"),format],[NSTextField(labelWithString:"Dynamic range"),hdrOutput],[NSTextField(labelWithString:"Color profile"),profile],[NSTextField(labelWithString:"Bit depth"),depth],[NSTextField(labelWithString:"JPEG quality"),quality],[NSTextField(labelWithString:"Longest edge (px)"),edge],[NSTextField(labelWithString:"Output sharpening"),sharp],[NSTextField(labelWithString:"Filename"),name]])
        grid.rowSpacing=12;grid.columnSpacing=12;grid.column(at:1).width=210;left.addArrangedSubview(grid)
        for (control,label) in [(format,"Export format"),(hdrOutput,"Export dynamic range"),(profile,"Export color profile"),(depth,"Export bit depth"),(quality,"JPEG quality percent"),(edge,"Longest edge pixels"),(sharp,"Output sharpening"),(name,"Filename template")] as [(NSControl,String)]{control.setAccessibilityLabel(label)}
    ...
    private func renderPreview(){
        ...
        operation.addExecutionBlock{[weak self,weak operation] in
            ...
            let image=Result{try autoreleasepool{var rendered=try ModernRenderer.render(source:item.url,recipe:settings.isHDR ? item.record.active.recipe:item.record.active.recipe.sdr);if settings.isHDR{rendered=try HDRTone.toneMapSDR(rendered)};var output=try ModernRenderer.prepareOutput(rendered,settings:settings);let scale=min(1,1100/max(output.extent.width,output.extent.height));output=output.applyingFilter("CILanczosScaleTransform",parameters:[kCIInputScaleKey:scale]);output=try ModernRenderer.outputPreview(output,settings:settings);return try ModernRenderer.display(SoftProof.apply(output,settings:proof))}}
